...
-- =============================================
-- BATCH 4: payment_requests, points_redemptions, posts, profiles, push_subscriptions, reel_comments, reel_likes, reels, reviews, self_ads, services, user_roles
-- =============================================
...
DROP POLICY IF EXISTS "Active ads viewable by everyone" ON public.self_ads;
...
CREATE POLICY "Active ads viewable by everyone" ON public.self_ads AS PERMISSIVE FOR SELECT TO public USING (status = 'active');

-- services (from service_categories + services)
...
CREATE POLICY "Services viewable by everyone" ON public.services AS PERMISSIVE FOR SELECT TO public USING (true);
...
CREATE POLICY "Super admins manage all services" ON public.services AS PERMISSIVE FOR ALL TO authenticated USING (has_super_admin(auth.uid())) WITH CHECK (has_super_admin(auth.uid()));
...
CREATE POLICY "Super admins manage all user_roles" ON public.user_roles AS PERMISSIVE FOR ALL TO authenticated USING (has_super_admin(auth.uid())) WITH CHECK (has_super_admin(auth.uid()));
...
CREATE POLICY "User badges viewable by everyone" ON public.user_badges AS PERMISSIVE FOR SELECT TO public USING (true);

-- wallet_transactions
DROP POLICY IF EXISTS "Users can view own transactions" ON public.wallet_transactions;
DROP POLICY IF EXISTS "Users can insert own transactions" ON public.wallet_transactions;
DROP POLICY IF EXISTS "Admins can view all transactions" ON public.wallet_transactions;
DROP POLICY IF EXISTS "Super admins manage all wallet_transactions" ON public.wallet_transactions;
...
CREATE POLICY "Admins can view all transactions" ON public.wallet_transactions AS PERMISSIVE FOR SELECT TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Super admins manage all wallet_transactions" ON public.wallet_transactions AS PERMISSIVE FOR ALL TO authenticated USING (has_super_admin(auth.uid())) WITH CHECK (has_super_admin(auth.uid()));
